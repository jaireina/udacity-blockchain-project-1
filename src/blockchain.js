  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Set previous block hash if there's at least one previous block
        if (self.height >= 0) {
          block.previousBlockHash = self.chain[self.chain.length - 1].hash;
        }
        block.timestamp = new Date().toString().slice(0, -3);
        block.hash = SHA256(
          JSON.stringify({
            hash: null,
            height: block.height,
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const fiveMinutesInSec = 5 * 60;
        if (currentTime - msgTime > fiveMinutesInSec) {
          reject("more than 5 minutes have passed");
        }
